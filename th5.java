 ...
 abstract class PhongHoc {
	...
	public void setMaPhong(String maPhong) throws Exception {
		if(maPhong.length() > 0)
			...
			throw new Exception("Ma phong khong đuoc đe trong!");
	}
	...
	public void setDayNha(String dayNha) throws Exception {
		if(dayNha.length() > 0)
			...
			throw new Exception("Day nha khong đuoc đe trong!");
	}
	...
	public void setDienTich(double dienTich) throws Exception {
		if(dienTich >= 50)
			...
			throw new Exception("Dien tich khong đuoc be hon 50!");
	}
...
xuatDanhSach(listPH.getAllPH());
	...
	public static int menu()
	{
		...
		System.out.println("## 4.  In thong tin phong hoc có n may tinh    ##");
...
PhongHoc ph1 = new LyThuyet("PH001", "Day A", 150, 10, true);
	...
	{
		inGachNgang1();
		System.out.printf("|%-15s|%-10s|%-15s|%-15s|%-15s|%-15s|\n", "Ma phong hoc", "Day nha", "Dien tich(m^2)", "So bong đen", "May chieu", "Danh gia");
		inGachNgang1();
	}
